...
void Entity::setAnimationAtlas(const std::map<Direction, std::vector<int>>& atlas) {
...
void Entity::handleEntityCollision(Entity* other)
{
    // Throwable hits enemy
    if (mEntityType == MISSLE && other->mEntityType == NPC)
    {
        this->deactivate();
        other->startHitFlash(0.2f, true);

        return;
    }

    if (mEntityType == MISSLE && other->mEntityType == BLOCK) {
      this->deactivate();
    }

    // Player collides with enemy
    if (mEntityType == PLAYER && other->mEntityType == NPC)
    {
        this->deactivate();
        return;
    }
...
void Entity::checkCollisionY(Entity *collidableEntities, int collisionCheckCount)
{
    for (int i = 0; i < collisionCheckCount; i++)
    {
        ...
        if (isColliding(collidableEntity))
        {
            ...
            //handles collision based on entity type
            handleEntityCollision(collidableEntity);
        }
        ...
        case WALKING:
            // Depending on where the player is in respect to their x-position
                // Change direction of the enemy
                    ...
                    if (mPosition.y > target->getPosition().y) mAcceleration.y = -50.0f;
                    else                                       mAcceleration.y = 50.0f;
...
void Entity::render()
{
    ...
    Color tint = WHITE;
    if (mEntityType == NPC && mHitTimer > 0.0f) tint = RED;
    DrawTexturePro(
        mTexture, 
