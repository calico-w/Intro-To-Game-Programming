...
enum EntityType   { PLAYER, BLOCK, NPC, MISSLE, NONE };
...
private:
    ...
    void startHitFlash(float duration, bool kill = true)
    {
        mHitDuration  = duration;
        mHitTimer = duration;
        mDeath = kill;
    }
...
public:
    ...
    void setAIType(AIType newType)
        { mAIType = newType;                       }
    void launchThrowable(Vector2 startPos, Direction direction, float speed);
